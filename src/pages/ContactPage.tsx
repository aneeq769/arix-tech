  ...
  return (
    <Box
      sx={{
        ...
        pb: { xs: "72px", sm: "84px", md: "100px" },
        px: { xs: 2, sm: 3, md: "5vw", lg: "80px" },
        position: "relative",
        overflow: "hidden",
      }}
    >
      ...
      <Box
        sx={{
          ...
          borderRadius: "50%",
          background: `radial-gradient(ellipse,${V}06,transparent 70%)`,
          pointerEvents: "none",
        }}
      />
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", lg: "1fr 1.6fr" }, gap: { xs: 6, lg: 8 }, alignItems: "start" }}>
        <Box sx={{ display: "flex", flexDirection: "column", gap: 2 }}>
          {CONTACT_ITEMS.map((c, i) => (
            <Box key={i} className="hover-card" sx={{ display: "flex", alignItems: "flex-start", gap: { xs: 1.5, sm: 2 }, p: { xs: "18px 16px", sm: "22px 24px" }, background: CARD, border: `1px solid ${BORDER}`, borderRadius: "14px", position: "relative", overflow: "hidden", "&::before": { content: '""', position: "absolute", inset: 0, background: `radial-gradient(ellipse at 20% 50%,${c.color}12,transparent 60%)`, opacity: 0, transition: "opacity .4s" }, "&:hover::before": { opacity: 1 } }}>
              <Box sx={{ width: { xs: 36, sm: 40 }, height: { xs: 36, sm: 40 }, borderRadius: "10px", background: `${c.color}12`, border: `1px solid ${c.color}30`, display: "grid", placeItems: "center", fontSize: { xs: ".95rem", sm: "1.05rem" }, flexShrink: 0, boxShadow: `0 0 14px ${c.color}20` }}>{c.icon}</Box>
              <Box sx={{ minWidth: 0 }}>
                ...
                <Typography sx={{ fontSize: ".92rem", color: "text.secondary", overflowWrap: "anywhere" }}>{c.val}</Typography>
              </Box>
            </Box>
          ))}

          <Box sx={{ p: { xs: "18px 16px", sm: "22px 24px" }, background: `rgba(16,185,129,.06)`, border: `1px solid rgba(16,185,129,.2)`, borderRadius: "14px", display: "flex", alignItems: "center", gap: { xs: 1.5, sm: 2 } }}>
            <Box sx={{ width: { xs: 36, sm: 40 }, height: { xs: 36, sm: 40 }, borderRadius: "50%", background: `rgba(16,185,129,.12)`, border: `1px solid rgba(16,185,129,.3)`, display: "grid", placeItems: "center", flexShrink: 0 }}>
            ...
            <Box sx={{ minWidth: 0 }}>
              ...
              <Typography sx={{ fontSize: ".92rem", color: G, overflowWrap: "anywhere" }}>Available for new projects</Typography>
            </Box>
          </Box>
        </Box>

        <Box component="form" onSubmit={handleSubmit} sx={{ background: CARD, border: `1px solid ${BORDER}`, borderRadius: { xs: "16px", md: "20px" }, p: { xs: 2, sm: 3, md: 5 }, position: "relative", overflow: "hidden", "&::before": { content: '""', position: "absolute", top: 0, left: "20%", right: "20%", height: "1px", background: `linear-gradient(90deg,transparent,${C},transparent)`, opacity: .4 } }}>
